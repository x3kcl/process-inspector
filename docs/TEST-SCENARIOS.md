...
| TS-GUARD-05 | tier 4: scope re-resolved at submit with drift shown; prod type-the-count; **unscoped destructive bulk refused outright**. **Landed (#100), `terminate-delete` only: backend `DestructiveBulkServiceTest` (refuse-unscoped, ADMIN hard-gate fires before resolution, drift `409 bulk-count-drift` on a wrong typed count, dev-only scope needs no count); frontend `DestructiveBulkWizard.test.tsx` (auto-preview, per-engine readout, PROD-gated submit, drift banner with "Refresh the scope"). `CorrectiveActionServiceTest` proves the bulk-item reauth/confirm-token skip (`executeBulkItem`) against a stale session.** | SPEC §6/§7 |
...
| TS-RBAC-03b | protect/unprotect (#165): ADMIN-per-engine floor is a real HTTP door (VIEWER/RESPONDER/OPERATOR refused); reason ≥10 required either way; already/not-protected named 409s; fail-closed audit compensation on both doors. **Landed: `ProtectedInstanceServiceTest` (rung 1, protect's fail-closed path) + `ProtectedInstanceRbacSpringTest` (rung 3, the HTTP door + unprotect's fail-closed path); frontend `ProtectModal.test.tsx` (ADMIN-only trigger, reason floor, mode wiring).** | R-SAFE-05 |
