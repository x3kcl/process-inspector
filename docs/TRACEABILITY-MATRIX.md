...
| R-SAFE-05 protected instances | TS-RBAC-03, TS-RBAC-03b | `CorrectiveActionServiceTest`, `ActionRbacGuardSpringTest` (read/enforce); `ProtectedInstanceServiceTest`, `ProtectedInstanceRbacSpringTest` (write, #165); `ProtectionGuardTest`, `ProtectedDefinitionRbacSpringTest` (definition-scope write + shared guard, #172); FE `intersection.test`, `ProtectModal.test.tsx`, `DefinitionVersionsPage.test.tsx` | L1·L3·UNIT-FE | ✅ instance + definition-key scope built; enforcement not yet extended to the ~9 other instance-verb call sites or bulk (#184) |
