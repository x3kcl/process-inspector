...
| R-SAFE-17 scope-filtered reads (S2) | `ReadScopeGate` + `SearchController`/`SearchService` + `TriageScopeProjector`/`TriageController` + `ResolveController`/`ResolveService` (#300) + `AuditController` + `BulkController`/`BulkJobService` (#296) + `RelatedBulkJobsService` (#329) | `ReadScopeGateTest` (L1: flag-off→null-unrestricted, global-grant→all, per-engine/per-tenant `overlaps()` narrowing, empty-grant→none), `SearchServiceScopeTest` (L2: out-of-scope engine labeled + **`verifyNoInteractions(flowable)` — never queried**, null-scope over-filters nothing), `TriageScopeProjectorTest` (L1: readable narrowing + re-summed roll-ups + drop no-scope group + PARTIAL group nulls DL/retrying split), `ErrorGroupCard.test.tsx` ("—" render for scoped split), `ResolveServiceScopeTest` (L2), `AuditScopeApiSpringTest` (L3: scoped JSON list + CSV lines + config-event fleet-ADMIN-only + unscoped-unaffected), `AuditEntryRepositoryScopeIT` (L4-DB: real Postgres proves the `findLogScoped` JPQL predicate), `BulkScopeApiSpringTest` (L3: scoped list/detail, recomputed `totalItems`, zero-intersection 404, unscoped-unaffected), `RelatedBulkJobsServiceTest` (L1: job touching no readable engine omitted, spanning job narrowed with recomputed `totalItems`/tallies, unscoped-unaffected), `IncidentScopeApiSpringTest` (L3: same three cases over real HTTP on `GET /api/incidents/{id}`) | L1·L2·L3·L4-DB | ✅ Search + triage **dashboard** + leak-views/trends (#126) + omnibox resolve (#300) + audit operations-log/CSV + bulk-job reads (#296) + the incident detail's related-bulk-jobs join (#329) all built (flag off by default, on under `oidc`); scoped RBAC not rung-3-reachable (dev ladder global-only) |
...
| R-SAFE-05 protected instances | TS-RBAC-03, TS-RBAC-03b | `CorrectiveActionServiceTest`, `ActionRbacGuardSpringTest` (read/enforce); `ProtectedInstanceServiceTest`, `ProtectedInstanceRbacSpringTest` (write, #165); `ProtectionGuardTest`, `ProtectedDefinitionRbacSpringTest` (definition-scope write + shared guard, #172); `CorrectiveActionServiceTest`, `FlowSurgeryServiceTest`, `BulkJobServiceTest` (enforcement extended to instance/task/job verbs, change-state, restart-as-new, bulk dispatch, #184); FE `intersection.test`, `ProtectModal.test.tsx`, `DefinitionVersionsPage.test.tsx` | L1·L3·UNIT-FE | ✅ instance + definition-key scope built and enforced across nearly every verb + bulk (#172, #184); remaining boundary is EDIT_VARIABLE/UNSTICK_EVENT + CMMN, deliberately out of scope |
...
| R-NFR-02 latency budget | TS-TRI-06, P4 | `scripts/perf-scenario-p1.js` (nightly `perf-p1` job) | — | ✅ (§C-10) |
...
| R-TEST-05 performance scenarios P1–P4 (TEST-STRATEGY §7) | P1 (10-stub-engine partial-failure fan-out), P2 (50k-row DLQ vs `dlq-scan-cap`), P4 (100 landing loads/5s) | `FanoutPartialFailureIT` (nightly job `fanout-partial-failure-p1`, issue #298); `DlqScanCapVolumeIT` (nightly job `dlq-scan-cap-p2`, issue #299); `scripts/perf-scenario-p1.js` (nightly job `perf-p1`, §C-10) | L2·perf | 🟡 P1/P2/P4 built; P3 (SSE/bulk soak) remains open — §C-8 |
...
| R-BAU-10 incident ledger (v2) | ingestion state machine + zero-state regression gate + episode lifecycle + occurrence upsert (partitioned business-key PK) + R-SAFE-17 scope-projected reads + resolve/reopen config-events + related-bulk-jobs join + the full engine arc | `IncidentLedgerServiceTest`/`IncidentLifecycleServiceTest`/`IncidentQueryServiceTest`/`RelatedBulkJobsServiceTest` (L1); `IncidentApiSpringTest`/`IncidentScopeApiSpringTest`/`IncidentLifecycleApiSpringTest` (L3); `IncidentLedgerIT` (L4-DB: V18 validate, create-ahead partitions, idempotent upsert, sampler-race skip-not-clobber, synthetic regression arc); `IncidentLedgerArcIT` (L4 engine: seed→OPEN→error-class retry-all→drain→resolve→reopen→re-resolve→zero-state armed→re-seed→REGRESSED, related-jobs on detail, `incident-resolve`/`incident-reopen`/`incident-regressed` audit rows); FE `sections/timeline/gate/duration/breakdown/drill.test` (incl. the occurrence-gap no-interpolation case) | L1·L3·L4·UNIT-FE | ✅ (both L4 suites LOCAL-ONLY relative to ci.yml's itClass — failsafe `*IT`, engine-harness doctrine — but run nightly: `IncidentLedgerIT` in `container-its`, `IncidentLedgerArcIT` in `engine-its`, issue #294; browser arc verified at S4+S5) |
...
| C-10 | Latency budget (P4 / R-NFR-02) | R-NFR-02, TS-TRI-06 | CLOSED — `docker/seed-reference.sh` (FIX-REF-01 generator: 3 defs x 3 versions x 4 failure signatures per engine, full TEST-SCENARIOS.md §1.6 envelope) and the nightly `perf-p1` job (own compose stack + BFF, seeds full scale, runs `scripts/perf-scenario-p1.js` via dockerized k6, uploads the summary JSON) both landed. NOT merge-blocking (nightly-only), same as the other perf/IT jobs. | — | resolved (issue #93) |
...
| C-15 | Error-group acknowledge L4 · leak-view age-from-engine-ts | R-BAU-01/02, TS-TRI-09/10 | Acknowledge is automated at L1 (predicate + guards/compensation) and L3 (door + fail-closed audit) with the collapse/badge shapes at UNIT-FE; the real-Postgres round trip (constraint set, wholesale re-ack replace) and the engine-fed resurface arc have no L4 proof. Leak-view engine-timestamp age math is still untested. | L4: acknowledge against real PG + live engines → collapses "Acknowledged (N)", resurfaces on +threshold/new version/expiry, audit rows verified; leak-view age from engine `startTime`. | P2 |
...
| C-19 | R-TEST-05 P1 fan-out under partial failure (10 stub engines, 3 slow, 2 timing out) | R-TEST-05, TEST-STRATEGY §7 | CLOSED — `FanoutPartialFailureIT` (`backend/src/test/java/io/inspector/perf/`) landed: 10 `StubFlowableEngine` instances (5 healthy, 3 slow, 2 hung, extending the P2 suite's reusable stub, issue #299/PR #322) registered via the config-pinned registry test seam (`application-it-p1fanout.yml`). Proves search P95 ≤ read-ms+2s under partial failure; the 2 timed-out engines are marked `ok=false` with an honest reason (ARCHITECTURE §2.3 — never a fabricated zero/healthy); each healthy engine's OWN recorded latency stays an order of magnitude below the degraded engines' (no cross-engine starvation via the `engine_fanout_duration_seconds` chokepoint metric); BFF `/actuator/health` stays responsive throughout (do-no-harm); the breaker opens on 2 consecutive real timeouts, fast-fails while open, and fully CLOSES once the stub is flipped back healthy; a genuinely-saturated slow engine's bulkhead never blocks a different engine's own permits. | Own nightly job `fanout-partial-failure-p1`, distinct from both `perf-p1` (P4) and `dlq-scan-cap-p2` (P2) — never `perf-scenario-p1.js`. | resolved (issue #298) |
