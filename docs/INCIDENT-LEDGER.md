...
| `GET /api/incidents/{id}` | VIEWER | ledger row + episodes + windowed occurrence series (clamped like `/api/triage/trends`) + LIVE `ErrorGroup` join at render (incl. read-only ack state). **Episode action attribution (#358 item 2, `EpisodeActionAttributionService`):** each episode carries `actionsDuringEpisode {count, byVerb, byOutcome, truncated}` — an honest audit-side aggregate of corrective actions whose engine + timestamp falls inside that episode's `[startedAt, endedAt-or-now]` window, sharing the identical engine+timestamp join RETRYING-RISK-LANE.md §3.3's `#351` self-heal confound detection uses (widened to every verb/outcome, narrowed to the episode's own precise boundaries). Aggregate-only (no engine id, instance id, actor, or reason) — never de-minimizes the audit payload (R-AUD-03); see RETRYING-RISK-LANE.md §3.3/§10 for the honest over-inclusion caveat and the still-open exact-per-instance follow-up |
