...
- **BFF-side over the scan legs** — `failureTimeAfter/Before` (inclusive window over
  ...
  A `signatureHash` filter also carries a `signatureAlgoVersion` stamp (#279): a KNOWN
  non-current generation matches no fresh hash by construction, so `SearchService.aggregate`
  short-circuits to zero rows **with a `SignatureGeneration` reason** on the response BEFORE any
  engine is touched (`signatureGenerationNotice`, a pure rung-1 seam) — the read-path analogue of
  the write path's 409 `error-class-algo-mismatch`, degrading not refusing. A legacy/unstamped link
  is assumed-unknown: it still runs (it may carry a current hash) but carries the same advisory so
  a zero is not misread (SPEC §4).
...
so does the blind-cycle marker: `incident_occurrence.cycle_complete` (V21) records whether
every registry engine was reached on the pass that wrote the row. A blind row's counts are
missing an unreachable engine's members, so it is a FLOOR-like non-level exactly as a
truncated one is — every derived reader discards a delta that touches it (the attention F
factor's positive-delta sum) and refuses to read a `retrying_count` edge at it as a spell
boundary (the R2 self-heal lane). Recording the flag on the SAMPLE only — as first shipped —
let a two-minute engine outage read downstream as hundreds of phantom arrivals and as a
fabricated SELF_HEALED spell. **Discarding is COUNTED, and it reaches the wire** (post-ship
correction, ALARM-COST-MODEL.md §13): the attention arrivals aggregate returns its sample
counts so a wholly-discarded window reports "unknown" (neutral F) rather than the zero it
arithmetically produces, and `IncidentDetail.OccurrencePoint` carries `cycleComplete` beside
`truncated` so the detail sparkline can mark a blind dip instead of drawing it as a recovery.
**Observation SCOPE is a separate axis from observation quality** (`incident_occurrence.fleet`,
V22/#372, ALARM-COST-MODEL.md §16): both markers above say how WELL the pass saw what it was
looking at; neither says WHAT it was looking at. A registry DISABLE removes an engine from
`EngineRegistry.all()`, so the pass never fans out to it and never fails to hear from it —
`cycle_complete` stays honestly TRUE while the class's level silently loses that engine's
members. `fleet` records the canonical sorted enabled-engine id set the pass fanned out over
(including engines that answered not-ok — in scope but unobserved is `cycle_complete`'s job), and
the derived readers require SAME-NON-EMPTY-fleet endpoints exactly as they require clean quality
markers: otherwise a re-enable banks a level shift as hundreds of arrivals, and a disable reads as
a RETRYING spell ENDING with no DLQ growth, i.e. a **SELF_HEALED fabricated by an admin action**.
Pre-V22 rows carry `''` = unrecorded, comparable to nothing (itself included) — scope at write
time cannot be reconstructed later, so it is never guessed. `IncidentDetail.OccurrencePoint`
carries `fleet` on the wire and the incident-detail read gained an optional `until` cursor so a
VIEWER can page backward past the 30-day window clamp to find an era boundary (each call stays
bounded to one clamped window). Frontend rendering of era boundaries is a deliberate non-goal.
...
| `GET  /api/triage[?refresh=true]` | Stage 0 dashboard: engine health strip (M1 probe state), global + per-engine status counts (query totals, `size=1`), DLQ + RETRYING jobs grouped by normalized error signature with per-engine/per-definition-version counts (sibling versions zero-filled), per-engine honesty envelope (`ok/error/dlqScan`, plus `outOfScopeDeadletters` — dead-letters belonging to a co-deployed engine sharing the job tables (CMMN), counted not dropped so the strip's dead-letter lane reconciles with FAILED; null when the engine cannot discriminate scope, gated on the `scopeType` capability ~6.8+). Served from the 20s Caffeine cache (single-flight); `refresh=true` bypasses, throttled 1/10s. The R-BAU-01 **acknowledge overlay** (`ErrorGroup.acknowledgement`: who/when/reason/ticket/expiry + `resurfaced`/`resurfaceReason`/`grownBy`) is joined onto the CACHED aggregation at render time from the `error_group_ack` store — the engine cache never carries ack state and is never busted by an ack/unack; rows from another `algoVersion` generation decorate nothing (R-SEM-03 needs-re-binding). **Attention ordering (v2 R1, #353, [ALARM-COST-MODEL.md](ALARM-COST-MODEL.md)) — FLAG-OFF by default:** cards are ordered `total DESC` (count-only). With `inspector.triage.attention-ordering=true` a THIRD render-time overlay joins after scoping and after ack decoration (`AttentionScoreService.decorate`, mirroring the ack decorator's never-mutate-the-cached-object contract): each `ErrorGroup` gains `attention {score, factors{frequency,recency,mttr,selfHeal,arrivals28d,ageSeconds,medianMttrSeconds?,closedEpisodes,selfHealLane?,insufficientHistory,arrivalsUnknown,discardedArrivalSamples,flooding,burstArrivals,burstWindowSeconds,burstUnknown,discardedBurstSamples}, rationale, suggestedAckExpirySeconds?}` and the list is re-sorted by `score DESC → total DESC → signatureHash ASC` (an R-SEM-23 total order). `A(c) = F·R·S` (**`M ≡ 1` since #399/§17** — see the `M` clause below): `F = log2(1+arrivals_28d)` from a DB-side positive-delta aggregate over `incident_occurrence` (deltas across a truncated bucket discarded — a floor is not a level, R-SEM-12 — and across a BLIND bucket, `cycle_complete=false`: an unreachable engine's drop-and-recover is an outage, not arrivals, #302/V21). The aggregate returns its SAMPLE COUNTS beside the sum, so a discard is never a silent zero: a window with samples but no trustworthy one reads `F = 1` (neutral, §4.1's degradation rule) with `factors.arrivalsUnknown` and a rationale clause saying so, because `F` is a FACTOR and a zero there erased the whole score — and since a group is truncated when ANY engine it touches capped, a permanently scan-capped engine used to zero exactly the fleet's LARGEST classes. The baseline is seeded at 0 for an incident's OWN first-ever row (joined on `incident.first_seen`), so a class's birth counts as the arrival of its whole population while a window merely STARTING mid-life still discards its first row — growth stays distinct from size. **Burst gate (#365, §4.1a):** the SAME single aggregate pass returns four more FILTERED columns — `burst_arrivals` over the trailing ISA-18.2 flood window W (`inspector.triage.attention.burst-window`, PT10M), `prior_burst_arrivals` over the window before it, and the current bin's own `burst_observed_samples`/`burst_trusted_samples` (zero new statements, zero new engine calls). A stateless two-bin Schmitt trigger fires when `burst_W ≥ burst-onset` (10) OR (`burst_W ≥ burst-exit` (5) AND `prior_W ≥ burst-onset`) — the hold leg needs a GENUINE prior onset, since summing the bins would gate 6+6 across 20 minutes as a 10-minute flood. When it fires, `F = log2(1 + outside_W + γ·burst_W)` with γ = `burst-weight` (8): a DECOMPOSITION of the arrivals already counted (`arrivals_28d = outside_W + burst_W`), so every arrival is banked exactly once at weight 1 or weight γ and a class born inside W counts its population once at γ — never a bolt-on multiplier over a sum that already contains it. Below the onset `F` is byte-identical to the shipped expression, so the neutrality guarantee is unmoved; the log self-bounds the inflation at `1 + log2(γ)/log2(1 + onset)` ≈ 1.87× (attained max 1.83×, shrinking as volume grows), the same order as the M clamp. Both bins inherit the truncation/blind-cycle trust discipline verbatim: a bin with samples but none trustworthy sets `factors.burstUnknown`, forces the gate OFF and leaves `F` at its un-boosted value — an unknown bin can only suppress a promotion, never demote. `burst-exit > burst-onset` is REFUSED at binding (an inverted Schmitt trigger has no sane reinterpretation) and `burst-window ≤ model-ttl` logs a startup WARN (the bins are evaluated at model-build time, so W must outlive one rebuild), `R = 2^(-age(lastSeen)/τ)` (τ = `attention.recency-half-life`, default 24h, deliberately the quiet-window constant), `M` is **NOT an ordering term** (post-ship correction #399, epic #398, ALARM-COST-MODEL.md §17): the estimator `clamp(medMTTR(c)/medMTTR(fleet), 0.5, 2)` over CLOSED `incident_episode` rows is RETAINED and still served on `factors.mttr` (neutral 1 below `attention.min-closed-episodes`, and the median is then ABSENT rather than rendered as a number), but the score multiplies by 1 instead — `medMTTR` is `ended_at − started_at` where the episode opens at the sampler's FIRST SIGHTING and closes only on the operator's S3 resolve verb, so it contains the operator's queue wait and is ENDOGENOUS to the ordering it fed (rank low ⇒ wait longer ⇒ MTTR rises ⇒ rank higher; inverting to `1/M` flips that loop positive, i.e. starvation, so both readings are refused). `min-closed-episodes` + `mttr-clamp-{low,high}` still shape the reported evidence and the §3.2 ack-expiry suggestion, and no wire field changed; the `AttentionBadge` glossary tooltip no longer claims resolve time weights the ranking, and `S = max(1 − p_heal·w, floor)` where `p_heal` is mapped from the R2 DISPLAYED (server-dwelled) `selfHeal.lane` — never a raw per-request rate, so the ordering cannot flap while the badge holds; `INSUFFICIENT_HISTORY`/absent lane ⇒ neutral 1. **Timing-aware since #400 (§4.1b):** `w = 2^(−t_heal/τ_heal)` over the already-served `selfHeal.ttsP50Seconds` (τ_heal = `attention.self-heal-horizon`, PT1H) — a class that heals *eventually* is no longer demoted as hard as one that heals *in a minute* (measured 80.73 % harm / 0.00 % help past ~8 service times). `w ∈ [0,1]` so the term can only ever WEAKEN a demotion, and at `t_heal = 0` (a heal inside one sampler bucket — the §3.1 resolution floor) `S` is byte-identical to the pre-#400 expression; an ABSENT `ttsP50Seconds` (`healed = 0`, or below the n < 10 sample floor) reads `w = 0` ⇒ `S = 1`, §4.1's degradation rule rather than an unevidenced "heals immediately". `attention.self-heal-floor` (0.25) is INERT at every value ≤ 0.25 — the "demoted at most 4×, never zeroed" bound is lane-quantisation arithmetic (`1 − p_heal(LIKELY)`), not the clamp — and is retained because a value strictly above 0.25 binds and weakens demotion (#400 part A). `eff(c)` is deliberately NOT a v1 factor (single-job attribution is blocked by R-AUD-03 payload minimization, so it would collapse to a fleet prior). Zero new engine calls; the Stage 0 count-only/`size=1` + dedicated-DLQ-scan rule is untouched. **Cost, re-measured 2026-08-04** (`org.hibernate.SQL=DEBUG` against the real pilot ledger, 7 scored classes): the attention MODEL is exactly three fleet-wide reads against the BFF's own Postgres cached whole for `attention.model-ttl` (5m) — the window-scoped `incident` fetch, the `incident_occurrence` arrivals aggregate and the closed-`incident_episode` durations — but the `S` factor is NOT in that model and NOT on that cache: it is consumed per CLASS from `SelfHealStatsService`, whose own TTL is the 60s sampler beat and whose miss path is three more BOUNDED reads (the incident row, the ≤5 000-row `RetryAuditPoint` confound projection, the ≤10 000-row spell-shape occurrence read). Measured `1 + 3 + 7×3 = 25` statements cold and **1 warm** — inside both TTLs the decoration costs nothing, and the 60s dwell tick normally pre-fills the per-class cache. (The older one-line claim "three bounded aggregates … cached whole" described only the model half.) **Ordering only, never hides:** a permutation plus a read-only overlay — no card filtered, no section membership changed, R-BAU-01 collapse/resurface semantics verbatim. Degrade-safe: any failure serves the undecorated response. Every factor degrades to a multiplicative identity when its evidence is missing, so with no ledger history every score is 0.0 and the tie-break reproduces today's count-only order exactly (measured across all 21,229 recorded pilot buckets, Kendall τ = 1.0 — proven by `AttentionOrderingNeutralityTest`) |
...
| `POST /api/triage/error-groups/acknowledge` · `…/unacknowledge` | **R-BAU-01 error-group acknowledge** (usability W3-2): BFF-store-only mutations — no engine call, ever (muting a card is not a corrective action on engine state). Body is coordinates only: `{signatureHash, algoVersion, reason (mandatory ≥10 both verbs), ticketId?, expiresAt? (ISO instant, future)}` — the BFF resolves the group's live engine × definition slices and their baselines (count + max FAILING version, zero-filled siblings ignored) from the SAME cached aggregation the operator is looking at, and persists one `error_group_ack` row per slice (V15; re-acknowledge replaces the signature's rows wholesale — fresh baselines). Rails: OPERATOR floor at the door + programmatic OPERATOR re-check per slice ENGINE; fail-closed `recordConfigEvent` (`error-group-acknowledge`/`error-group-unacknowledge`, reason in the COLUMN) with store compensation on audit failure (503, R-AUD-10); never auto-retried. Refusals: stale `algoVersion` ⇒ 409 `error-class-algo-mismatch`; drained/absent group ⇒ 409 `error-group-absent`; un-ack of an unacknowledged group ⇒ 409 `error-group-not-acknowledged`; short reason / bad expiry ⇒ named 400s. Resurface predicate (`ErrorGroupAckPolicy`, threshold default +20% via `inspector.triage.ack-resurface-threshold-pct`): new failing version or uncovered slice ⇒ `new-version`, growth past baseline·(1+pct) ⇒ `grew`, passed expiry ⇒ `expired`. The threshold's PROVENANCE moved to `ResurfaceThresholdEstimator` (#353, ALARM-COST-MODEL §3.3 / C3) — behaviour unchanged by default: the model-derived per-class value (`t(c) = max(ack-resurface-threshold-pct, resurface-floor-pct, k·CV(c))`, `k` fit by counterfactual-ack replay over `incident_occurrence` so it is computable at ZERO recorded acks — and a replay whose SMALLEST grid candidate fires no resurface at all is UNFITTABLE rather than trivially satisfied, so opting in can only ever RAISE the guard, never halve it) is gated behind `inspector.triage.attention.derived-resurface-threshold` (**default false** — the design activates it only after the full data-maturity gate, whose ack-lifecycle axis stands at zero), and every thin-evidence or failure path falls back to the same constant. R-BAU-01 semantics themselves are untouched |
| `GET  /api/incidents?state=&window=` | **R-BAU-10 incident ledger list** (v2, §2.7, [INCIDENT-LEDGER.md](INCIDENT-LEDGER.md) §6): the bounded, UNPAGINATED ledger — cardinality is distinct failure classes (tens-to-hundreds); sections (REGRESSED/OPEN/QUIET/RESOLVED + generation split) derive client-side from the full list. Each `IncidentSummary`: identity (`signatureHash`+`algoVersion`, plus `currentGeneration` computed against `ErrorSignatureNormalizer.ALGO_VERSION` at read time so the UI splits generations without hardcoding the normalizer version), `state`, first/last seen, latest `lastTotal`+`lastTruncated` (R-SEM-12 lower bound), the `countsByEngine` breakdown (jsonb parsed back to `engineId → "defKey:vN" → count`), regression counters, and `quiet` — DERIVED per read (`lastSeen < now − inspector.incidents.quiet-window`), never stored. `state=` filters case-insensitively (unknown ⇒ 400 ProblemDetail); `window=` (hours, server-clamped to 30 days like `/api/triage/trends`) keeps only incidents last seen inside the window. VIEWER floor. Served by `IncidentQueryService` off the ledger store alone — reads stay live under `inspector.incidents.enabled=false` (that flag gates INGESTION, not reads: written history remains inspectable). **Scope-filtered reads (R-SAFE-17):** service-layer projection off the same `ReadScopeGate` set as the dashboard/trends (the list is bounded/unpaginated, so the `TriageScopeProjector` doctrine, not a SQL predicate — INCIDENT-LEDGER §0): `countsByEngine` narrowed to readable engines; a zero-intersection incident is OMITTED; a partially-scoped one follows the `LeakDefinitionCount` honesty precedent (recompute, never null — an incident total decomposes fully per engine, unlike `ErrorGroup`'s DL/retrying split): its display total is RECOMPUTED from surviving engines + `partial=true`, the fleet `lastTotal` carried only when fully in scope; `lastTruncated` stays the fleet observation (a conservative floor marker). Off ⇒ legacy fleet-wide reads. **Hard cap + honest truncation (issue #308, INCIDENT-LEDGER §6):** the response is `IncidentListResponse{items, truncated}`, not a bare array — the store fetch is ALWAYS capped at `inspector.incidents.list-cap` (default 500) + 1 rows ordered `lastSeen DESC`, so an unbounded query is structurally impossible even when `window` is absent; a truncation always drops the OLDEST rows (the freshest possible slice survives) and `truncated=true` says so; the frontend badges it (§2.3) and never renders a capped list as complete — the no-pagination-v1 decision itself is unchanged, "absent window" still means "the whole ledger", just bounded by the cap now instead of truly unbounded. **Self-heal statistics (v2 R2, #351, [RETRYING-RISK-LANE.md](RETRYING-RISK-LANE.md)):** `IncidentSummary` additionally carries an optional `selfHeal` block `{lane, n, healed, wilsonLow?, wilsonHigh?, ttsP50Seconds?, ttsP90Seconds?, excludedSpells, truncationTainted}` — derive-on-read from the ledger's own `incident_occurrence` series (RETRYING-spell extraction) plus audit-side confound detection (successful `retry-job` rows on an engine the class touches, read as an engine+timestamp PROJECTION so the audit payload is never hydrated — R-AUD-03), Caffeine-cached with a TTL aligned to the sampler beat (`SelfHealStatsService`, zero new engine calls, zero new tables). Both per-class reads are BOUNDED: the occurrence read is a DB-side `LAG` filter returning only spell-shape-relevant rows (the spell, its closing zero, the +1 look-ahead) under a hard cap, and the confound scan is capped too — a cap drops the OLDEST rows, so a spell straddling the cut is LEFT-CENSORED and excluded rather than mis-judged. A spell whose observed shape contains a blind sample (`cycle_complete=false`) is gap-voided and never judged: an unreachable engine forges a `retrying_count` edge that would otherwise be recorded as SELF_HEALED evidence (#302). `lane` is the SERVER-side dwelled/hysteresis-stabilized DISPLAYED lane (never a raw per-request recomputation — a refresh cannot reset it, and two operators see the same lane), ticked once per COMPLETE sampler cycle off the same `AggregationSampledEvent` `IncidentLedgerService` consumes. `wilsonLow`/`wilsonHigh`/`ttsP50Seconds`/`ttsP90Seconds` are absent below the 10-unconfounded-completed-spell floor (`inspector.selfheal.floor`) — `INSUFFICIENT_HISTORY` is the default, expected-for-a-long-time lane. Gated only for computation by `inspector.selfheal.enabled` (default true); reads stay live regardless, exactly like `inspector.incidents.enabled`. Informational only — never referenced by `CorrectiveActionService` or any guard. **Attention score (v2 R1, #353):** `IncidentSummary` also carries the optional `attention` block described on the `GET /api/triage` row, present only under `inspector.triage.attention-ordering`. The ledger list keeps its SERVER ordering (`lastSeen DESC` — the #308 hard cap must drop the OLDEST rows) and its client-derived REGRESSED/OPEN/QUIET/RESOLVED sections; the score orders WITHIN the live sections, where they are actually formed. The rationale quotes the SCOPE-PROJECTED total, never the fleet number (R-SAFE-17) |
| `GET  /api/incidents/{id}?window=&until=` | **Incident detail**: the list-item `IncidentSummary` (embedded — one shape for card and header) PLUS `episodes` (ALL, newest first: start state/started/ended/resolvedBy/reason/ticket/peak, `durationSeconds` derived when ended — the per-episode MTTR substrate). **Episode action attribution (v2 R-BAU-10 follow-up, #358 item 2):** each episode additionally carries `actionsDuringEpisode {count, byVerb, byOutcome, truncated}` — an audit-side aggregate of corrective actions attributed to that episode's `[startedAt, endedAt-or-now]` window, closing the gap "per-class intervention outcomes are underivable from any read API" (RETRYING-RISK-LANE.md §3.3, #351's own confound-detection javadoc). Shares the EXACT engine+timestamp join #351's self-heal confound scan uses (`EpisodeActionAttributionService` mirrors `SelfHealStatsService`'s audit-side pattern, both reading a constructor-projection so `payload` never enters the SELECT — R-AUD-03 holds structurally), widened from "successful `retry-job` only, ±2 buckets" to "every verb/outcome, the episode's own precise boundaries". Over-inclusive by design (same accepted limitation as the confound rule): an action on the incident's CURRENT engine set during the window counts even if it targeted a different class active on that engine at the same time — exact per-instance attribution would need a signature stamped onto the audit row at write time, which this slice deliberately does NOT add (`CorrectiveActionService` and the audit payload are untouched). `byVerb`/`byOutcome` are aggregate tallies only (no engine id, instance id, actor, or reason); `truncated` is honest per a 5 000-row-per-episode cap. Zero new engine calls, zero changes to any corrective-action rail. PLUS the occurrence `series` inside `window=` (hours, same 30-day clamp, default 24; `seriesWindow` echoes the clamped ISO duration; ascending `{sampledAt,total,deadLetterCount,retryingCount,truncated,cycleComplete,fleet}` — a truncated point is a FLOOR, not a dip (R-SEM-12) and a `cycleComplete=false` point is BLIND, written while an engine was unreachable so its dip is an OUTAGE and not a drain (#302/V21). BOTH quality markers ship, and the SPA marks both on the detail sparkline with distinct shapes plus their own legend lines — never colour alone; shipping `truncated` alone let a blind-cycle dip render unbadged, which is the same iron rule violation as rendering a status derived from truncated data without the badge. `fleet` (V22/#372) is the row's observation SCOPE, not a third quality marker: the canonical sorted enabled-engine ids the writing pass fanned out over, `''` when unrecorded. Two points are difference-comparable only when both carry the same non-empty fleet — a registry disable/enable moves a multi-engine class's LEVEL with both quality markers clean. It ships so the VIEWER/REST-only G5 era-boundary walk can see boundaries; rendering them is a deliberate non-goal. The optional `until=` CURSOR (ISO-8601 instant, #372) moves the window's upper bound so a caller can page BACKWARD past the 30-day clamp in bounded chunks — each call is still limited to one clamped window, only the reachable TOTAL span changes; omit it and the read is byte-identical to the pre-#372 one) PLUS `live`: the CURRENT `ErrorGroup` for `(signatureHash, algoVersion)` joined at render from the SAME shared cached aggregation the dashboard serves — `TriageService.dashboard(false)` → `TriageScopeProjector` → ack decoration, exactly the `GET /api/triage` pipeline; a cold cache runs the shared single-flight aggregate (the dashboard's own cold path), NEVER a second fan-out plan; omitted when drained, generation-retired, or scoped away. PLUS `relatedBulkJobs` (S5): the most recent error-class bulk retries whose SUBMIT matched this `(signatureHash, algoVersion)` — a READ-ONLY join whose join table is the submit's ENVELOPE audit row (`payload.errorClass.*` + `payload.bulkJobId`; the `bulk_job` row's V4 scope descriptor is a human label and never carried the signature), newest first, bounded (10), batch-reading the job/item stores (`RelatedBulkJobsService` — never per-job item queries); each entry mirrors the `GET /api/bulk` list-item field set (id/verb/state/actor/timestamps/scope descriptor/per-item tallies, item rows stay on the bulk surface); this join reads `BulkJobRepository`/`BulkJobItemRepository` directly (not through `BulkJobService`), so **it threads its OWN R-SAFE-17 scope narrowing (issue #329)** rather than inheriting the bulk surface's: `RelatedBulkJobsService` receives the SAME `readableEngineIds` the incident detail resolved for its own zero-intersection check (below) and applies the IDENTICAL `BulkJobService#recent`/`#get` doctrine (issue #296) — a job touching no readable engine is OMITTED from `relatedBulkJobs` entirely, a job spanning both a readable and an unreadable engine is NARROWED (`totalItems`/`tallies` recomputed from the visible item subset only, via `BulkJobItemRepository#findByJobIdInAndEngineIdIn`), never the fleet-wide signature match that used to surface an out-of-scope engine's bulk-job metadata here. Same scope projection as the list; unknown id AND zero-scope-intersection answer the SAME 404 ProblemDetail (never leak existence — the `ViewsController` saved-view doctrine, deliberately not a 403). VIEWER floor |
| `POST /api/incidents/{id}/resolve` | **R-BAU-10 S3 resolve** (§2.7, [INCIDENT-LEDGER.md](INCIDENT-LEDGER.md) §6): OPEN/REGRESSED → RESOLVED — a **config-event, not a corrective action** (the R-BAU-01 acknowledge's endpoint class: a BFF-store-only ledger claim, ZERO engine calls, so no typed confirmation/capability gate — while the ack door's operation rails hold verbatim: OPERATOR floor, reason ≥10 (400 `reason-too-short`), optional `ticketId`, fail-closed `recordConfigEvent` (`incident-resolve`; reason in the COLUMN, payload = incidentId/signatureHash/algoVersion/stateFrom→stateTo/reason/ticketId/episodeId) with store compensation on audit failure ⇒ 503, never auto-retried). Closes the live episode (`ended_at`/`resolved_by`/`resolve_reason`/`ticket_id` land on the EPISODE, §2.7) and resets `seen_zero_since_resolve` so the regression gate arms FRESH. Every transition is a state-conditional native UPDATE bumping `version` (the S1 sampler-race doctrine): already-RESOLVED ⇒ 409 `incident-already-resolved`; a raced transition ⇒ retryable 409 `incident-state-conflict` — never a clobber, never a 500. `alsoAcknowledge=true` (explicit opt-in, panel P3) composes the EXISTING acknowledge door strictly AFTER the committed+audited resolve as a SECOND separately-audited action; the ack keeps ITS per-engine OPERATOR re-check while resolve needs plain fleet-wide OPERATOR (deliberate asymmetry: a ledger claim, not an engine-surface mute), and any ack refusal is reported per slice in `IncidentResolution.acknowledgements` — NEVER rolling the resolve back. Unknown id AND zero-scope-intersection answer the read doors' SAME 404 (a scoped OPERATOR can neither mutate nor probe what they cannot see) |
| `POST /api/incidents/{id}/reopen` | **R-BAU-10 S3 reopen** — the human undo ("resolved by mistake"), distinct from the automatic REGRESSED transition: RESOLVED → OPEN with the LAST episode going live again (`ended_at`→NULL, resolve metadata cleared) instead of a new one being minted, and `regression_count`/`last_regressed_at` deliberately untouched (an undo is not a regression). Body `{reason ≥10}` — a flagged S3 deviation from the design table's body-less row (audit doctrine: the why lands in the audit row's reason column, the un-acknowledge precedent). Same endpoint class + rails as resolve: OPERATOR floor, fail-closed `incident-reopen` config event (same payload shape), state-conditional (non-RESOLVED ⇒ 409 `incident-not-resolved`; raced ⇒ retryable 409 `incident-state-conflict`), same 404 scope semantics |
...
| `POST /api/instances/{engineId}/{id}/migrate/preview` | v2 instance migration (SPEC §5 tier 3): the **BFF static auto-map check** — Flowable's REST API exposes NO migration validator (P0 spike 2026-07-09), so this is an Inspector estimate, NEVER an engine validation (`engineValidated:false`). Runs every blocking guard (ADMIN floor, migration cap ≥6.5, writable, instance running + NOT suspended, target resolve+pin: same-key else 422, cross-tenant 422, same-version 409) then diffs the instance's ACTIVE activities against the target model by id + type + nesting + **source node kind** → `AUTO_MAPPED`/`FLAGGED_UNMAPPED`/`TYPE_CHANGED` (loud silent-corruption warning)/`NESTING_CHANGED`/`SCOPE_REMOVED`/`BOUNDARY_REMOVED` (the last two non-blocking), the exact `{toProcessDefinitionId, activityMappings}` document execute would send, an `activityStateDigest`, the honesty banner. Each entry carries typed **`findings[]`** and the response an instance-level **`findings[]`** (INSTANCE-MIGRATION.md §14: a CLOSED 8-code vocabulary — `UNMAPPED_ACTIVE_ACTIVITY` and `SCOPE_COLLAPSE_TOKEN_LOSS` (the two `BLOCKER_ADVICE`s); `ACTIVE_SCOPE_REMOVED`, `ACTIVE_IN_REMOVED_SCOPE`, `NESTING_PATH_CHANGED`, `TYPE_CHANGED_SAME_ID`, `BOUNDARY_SUBSCRIPTION_REMOVED` (`WARNING`); instance-level `BOUNDARY_CLOCK_RESET` (`INFO`)). `SCOPE_REMOVED` splits on a token COUNT taken from the active-execution MULTISET (§14.11): ≤1 live token inside the dissolving scope ⇒ `ACTIVE_SCOPE_REMOVED` warning; ≥2 ⇒ `SCOPE_COLLAPSE_TOKEN_LOSS` blocker (the engine 200s and destroys all but one). Every predicate is computed from data this call already fetched: **zero new engine calls**. `blocker`/`warning` are DERIVED from the findings, so severity has one source — the status is never a severity. Read-only: no audit row |
| `POST /api/instances/{engineId}/{id}/migrate/execute` | The migration (tier 3; ADMIN floor every env, typed **business-key** confirm on prod): re-plans server-fresh, asserts the §5 compare-and-set (`expectedFromDefinitionId` + `expectedActivityStateDigest`, both MANDATORY — 400 `preview-required` if absent, 409 if the instance moved since preview), then protection guard, reason ≥10, refuses if any activity still carries a `BLOCKER_ADVICE` finding — 422 `scope-collapse-token-loss` (a dissolving scope with ≥2 live tokens inside; unresolvable, so checked first) or 422 `unmapped-activities` (`FLAGGED_UNMAPPED`: the BFF has nothing sendable for it) — or >200 active executions (422 `instance-too-large`), fail-closed audit PENDING carrying the `migrate-instance/v2` document (`engineValidated:false`, from/to pinned, `activityStateDigest`, typed `bffFindings` + `taxonomyVersion` replacing v1's `bffWarnings`), ONE `POST …/migrate`, never retried (post-dispatch timeout ⇒ UNKNOWN + verify-now), re-reads the instance to record the observed definition. IRREVERSIBLE |
...
| `GET  /api/bulk/events` | SSE (v1.x #2, live-ui-sse doctrine): ONE stream per browser; id-only `bulk-job` events (data = job UUID) bridged from `BulkJobChangedEvent` + a 15 s `ping` heartbeat; no initial event on connect (clients fetch their own first state); a failed write drops the emitter, never `complete()`s it. Auth = session cookie (EventSource cannot send Authorization; the dev chain persists Basic auth into the HTTP session). On shutdown the hub completes every stream BEFORE the web server's graceful-shutdown lifecycle phase — an open stream must not hold the 30 s grace period hostage. **Non-blocking dispatch + coalescing (issue #301):** `SseHub.send` never runs on the publishing thread (a bulk item worker holding an engine dispatch permit, or the heartbeat scheduler) — it snapshots the emitter list and hands each write off to its OWN virtual thread, serialized per emitter via a `ReentrantLock` (never `synchronized` around blocking I/O — CLAUDE.md's Java-21-VT rule), so one stalled browser tab's blocking write pins only that carrier and never delays a sibling subscriber or paces engine dispatch; `stop()` takes the same per-emitter locks before completing, preserving "an errored emitter is NEVER completed". `onBulkJobChanged` never calls `send` itself — it schedules a coalesced flush per job id (`inspector.bulk.sse-coalesce-ms`, default 250 ms) so a burst of per-item events for one job collapses to one flush. Subscribers are capped (`inspector.bulk.sse-subscriber-cap`, default 200); beyond the cap a subscribe attempt is completed immediately rather than growing the registry unbounded, and the browser's own EventSource reconnection degrades the UI to polling (OPERATIONS §2 / RUNBOOK §7). Engine-health events join this stream in a later slice |
| `GET  /api/bulk[/{id}]` · `POST /api/bulk/{id}/cancel` | Job state machine (`PENDING→RUNNING→COMPLETED\|CANCELLED\|INTERRUPTED`) + persisted per-item report (tallies on the list read, items on the detail read); cancel stops DISPATCHING — sent items keep their outcome; startup sweep marks stale PENDING/RUNNING → INTERRUPTED (dispatched→`unknown`, pending→`not_run`, never re-fired); "continue as new job" = a fresh submit with `continuedFrom`. **Scope-filtered reads (S2, R-SAFE-17, issue #296):** `recent`/`get` resolve `readableEngineIds` on the request thread and push it into the repository queries — a job with no item on a readable engine is omitted from the list; a partially-in-scope job keeps appearing with its item list/tallies narrowed to the visible subset and `totalItems` recomputed from the survivors; a detail read with zero visible items answers the same 404 as an unknown id. Off ⇒ legacy fleet-wide reads |
...
| `GET  /api/instances/{engineId}/{id}/nearest-sibling` | **Sibling diff** auto-suggest (v1.x #5, landed): the smart default — most recently COMPLETED instance of the same `processDefinitionId` (`finished:true` + `sort=endTime desc`, reached an end event, not dead-lettered); returns `found:false` (not an error) when a fresh version has no completed run. Also carries `candidates`: every non-self row of the same scan (endTime desc, cap 25, `sibling` is its head) so the Compare tab's picker offers a dropdown of recent siblings without a second engine query; the manual id input stays the escape hatch (older runs, cross-definition, failed auto-suggest). VIEWER floor, historic queries only |
...
| `POST /api/playbooks` · `GET /api/playbooks` · `POST /api/playbooks/{id}/replay` | **Remediation playbooks** (v2): record = distill the exemplar's audit rows into a verb sequence; replay = a bulk job whose items run the sequence with per-step precondition rechecks. NOT YET BUILT — gated behind the S0 measurement below (R-GOV-08); "do not start 2/3 before 1 produces evidence of demand" |
| `GET  /api/admin/remediation-demand` | **Remediation-demand S0 measurement (issue #106, R-GOV-08)** — the ONLY landed slice of the playbooks feature above. Computed on demand, nothing persisted or scheduled: mines `audit_entry` (paginated, 500/chunk, capped at 50 000 rows, `truncated` flag if hit) for instance-scoped rows, per-instance collapses consecutive-duplicate verbs, and counts distinct instances per adjacent verb-pair ("signature" isn't captured on audit rows, so this is a verb-pattern proxy — documented in `RemediationDemandAnalysisService`'s own doc comment, not true error-signature grouping). `demandTriggerFired` = a ≥90-day data span AND at least one sequence on ≥10 distinct instances — R-GOV-08's two conditions as a conjunction. ADMIN-gated (the findings surface cross-engine operator behavior patterns) |
| `GET  /api/admin/audit/integrity` | **Chain-walk tamper-evidence verifier** (issue #304): `AuditChainVerifier` walks `audit_entry` in `seq` order (keyset-paginated, bounded batch — never a full in-memory load, capped total rows with a `rowCapHit` flag) and recomputes each row's `chain_hash` from its actual stored predecessor hash, reporting the first mismatch/gap plus a summary (`rowsChecked`, `partitionsSpanned`, `firstProblemSeq`). A `seq` gap is checked against `AuditRetentionPurger`'s pre-drop checkpoint config event — a match is a `RETENTION_BOUNDARY` finding (expected, not tampering), no match is `UNEXPLAINED_GAP`. Recomputation canonicalizes the read-back `payload` (jsonb reorders/reformats on storage — `AuditService.canonicalizePayload` pre-sorts to jsonb's own key order at write time, `AuditChainVerifier` reserializes compactly on read) so a genuinely intact row always re-verifies. ADMIN-only, and itself an AUDITED read (`audit-integrity-check` config event, mirroring `/api/admin/access`'s `access-read`). See OPERATIONS §6/§7 for exactly what this chain does and does not prove (unkeyed SHA-256 — HMAC keying is issue #311, PO-gated) |
| `GET  /api/audit` | Global operations log (filterable). **Scope-filtered reads (S2, R-SAFE-17, issue #296):** `readableEngineIds` resolved on the request thread and pushed into the repository query as an `engine_id IN (...)` predicate (`AuditEntryRepository#findLogScoped`) — never filtered in memory afterward. A config-event row (`_inspector` sentinel, R-AUD-10) is visible ONLY to a fleet ADMIN under enforcement (the same safe default as its existing payload gate). Off ⇒ legacy fleet-wide reads |
| `GET  /api/audit/export` | Operations-log **CSV export** (R-AUD-08): the SAME filters as `GET /api/audit` (actor/action/engineId/ticketId/since), streamed as `text/csv` in repository pages (500/chunk, 10 000-row ceiling), attachment disposition. Skeleton columns only — payload/response bodies never travel (R-AUD-03); every text cell RFC-4180 + formula-escaped (R-OPS-08, `Csv.cell` shared with the access-review export). VIEWER floor. **Scope-filtered reads (S2, R-SAFE-17, issue #296):** the SAME `engine_id IN (...)` predicate as `GET /api/audit` is pushed into EVERY page query — the export previously took no `Authentication` parameter at all, so per-row gating was structurally impossible here. Off ⇒ legacy fleet-wide reads |
...
| `GET·POST /api/admin/engines` · `PUT·DELETE /api/admin/engines/{id}` · `POST …/{id}/probe`·`/enable`·`/disable`·`/purge` | **Registry CRUD** (v2, REGISTRY-CRUD.md §9): runtime engine lifecycle. `GET` returns the full registry incl. tombstoned rows + auth/timeout/cap config + secret-ref **presence** (never values — distinct from the secret-free, display-shaped `GET /api/engines`, which carries live rows' health + `mode`/`lifecycle` only). `POST`/`PUT` add/edit (all fields except the immutable `id`), SSRF-validated (resolve-then-pin, egress allowlist, metadata/private denylist), audited fail-closed before/after. `probe` runs the **read-only** version+capability probe (no mutating call, transitions DRAFT→PROBED). On failure the row's `lastProbeFailureClass` distinguishes `unreachable`/`missing_secret_ref`/`auth_rejected`/`unexpected_response`/`ssrf_rejected` as VISIBLE row text (#275, REGISTRY-CRUD.md §2/§9 — a coarse, exception-TYPE-only class, never the raw message); an `active` row's `reachableNow` mirrors the same live `EngineHealthService` result the header strip uses, `null` until first checked. `enable?mode=read-write` on a prod engine ⇒ typed token + four-eyes (R-SAFE-08). `DELETE` soft-tombstones (requires DISABLED); `purge` hard-removes after retention. ALL gated `@rbac.canAdministerRegistry` (REGISTRY_ADMIN, fleet-scoped — per-engine ADMIN does not confer it) |
...
- **RBAC:** `VIEWER` (read-only) → **`RESPONDER`** (tier-0 verbs + unstick + notes — the
  ...
  the fan-out read aggregators (search, the omnibox resolve — issue #300, the triage dashboard,
  leak-views and trends — issue #126 closes out the last two, the audit operations log/CSV
  export and the bulk-job list/detail — issue #296 closes out the last two residuals)
  intersect the caller's grants at
  VIEWER via `ScopeGrant.overlaps`
  ...
  `/api/triage/trends` entries above have the per-endpoint detail. **Audit + bulk reads scoped too
  (issue #296, ★ BUILT 2026-07-26):** `AuditController.operationsLog`/`operationsLogCsv` and
  `BulkController.recent`/`get` were the last fleet-wide fan-out reads at the VIEWER floor — the
  CSV export previously took no `Authentication` parameter at all, so per-row gating was
  structurally impossible there. `readableEngineIds` is resolved on the request thread and pushed
  into the repository query itself as an `engine_id IN (...)` predicate (`AuditEntryRepository
  #findLogScoped`, `BulkJobRepository#findRecentByReadableEngineIds`,
  `BulkJobItemRepository#findByJobIdAndEngineIdInOrderByOrdinal`) — never filtered in memory after
  fetching, since the CSV export pages the repository directly and an in-memory filter would break
  its page-boundary math. A bulk job carries no engine of its own (its items do — a job can span
  several engines), so "readable" means "has at least one item on a readable engine"; a job
  touching NO readable engine is omitted from the list, and a detail read with zero in-scope items
  answers the SAME 404 as an unknown id (existence is never leaked, the `IncidentQueryService`
  zero-intersection doctrine); a partially-in-scope job keeps appearing with its item list/tallies
  narrowed to the visible subset and `totalItems` RECOMPUTED from the survivors (never the fleet
  count — the `LeakDefinitionCount` "recompute, never null" precedent). **The incident detail's
  related-bulk-jobs join scoped too (issue #329, residual off #296, ★ BUILT 2026-07-26):**
  `RelatedBulkJobsService` reads `BulkJobRepository`/`BulkJobItemRepository` directly (never
  through `BulkJobService`), so it did not automatically inherit the narrowing above — a
  caller who could see an incident (its own zero-intersection scope check gates that) could
  still see a related bulk job's full metadata even when that job touched an engine outside
  their scope. Fixed by threading the SAME `readableEngineIds` `IncidentQueryService` resolves
  for the incident's own projection into `RelatedBulkJobsService#forSignature`, applying the
  IDENTICAL omit/narrow doctrine via a new batched scoped query
  (`BulkJobItemRepository#findByJobIdInAndEngineIdIn`). **Config-event rows
  tighten under enforcement:** the `_inspector` sentinel (R-AUD-10) carries no engine of its own,
  so a scoped VIEWER's readable set never contains it by construction — under
  `scope-reads-enforced=true` a config-event row is visible ONLY to a fleet ADMIN (the same safe
  default as its existing payload gate); enforcement OFF leaves the legacy row-visible-to-any-
  VIEWER/payload-ADMIN-gated behaviour unchanged (IDP-SECURITY.md has no conflicting wording).
