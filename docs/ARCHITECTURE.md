...
- **RBAC:** `VIEWER` (read-only) → **`RESPONDER`** (tier-0 verbs + unstick + notes — the
  ...
  **`ACCESS_ADMIN`** (v2, IDP-SECURITY.md §9) is the **apex** orthogonal fleet grant — it
  ...
  it reads the OIDC session's `auth_time` and refuses a stale/absent-`auth_time` session on a tier-3
  verb with a `401 reauth-required` challenge (before reason/typed-token/audit — the SPA re-auths at
  verb intent and replays); non-OIDC sessions (dev Basic re-auths every XHR; break-glass can't bounce
  a down IdP) are exempt. Bulk-submit + mapping-write re-auth reuse the same gate (follow-up).
