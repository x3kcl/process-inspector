...
**Frontend npm-audit gate — dated-exception allowlist (issue #297).** A bare
`npm audit --audit-level=high` goes permanently red the instant any high+ advisory has no
in-range fix, training the team to ignore the job — worse than no gate. The `supply-chain`
job instead runs `scripts/check-npm-audit-allowlist.mjs` (Node, runnable locally the same
way): it diffs `npm audit --json` against the checked-in `frontend/npm-audit-allowlist.json`
and fails on either (a) a high/critical advisory with no matching entry, or (b) any allowlist
entry whose own `expires` date has passed — even if the advisory is still unfixed, so an
exception can never rot silently; someone must re-check for a fresh in-range fix and either
renew (new date + justification) or escalate. A patchable advisory is never allowlisted — it's
just fixed (`npm audit fix`) and drops out of the audit entirely; the allowlist is only for
advisories confirmed to have no safe in-range fix at review time. **Cadence:** review the
allowlist whenever the gate goes red on an expiry, and at minimum each monthly adversarial
review sweep (`deferred-work-tracking` cadence) — re-run `npm view <pkg> versions` for every
...
differently today: WireMock proves pure HTTP-client behavior in `GuardedCallerTest`, and the
live `integration` matrix's flowable-6/7/legacy profiles exercise real error-JSON shape
end-to-end — but no standalone fixture suite pins it).
