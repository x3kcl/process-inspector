...
| **Data security** (Art. 8) | TLS to engines and clients; RBAC — audit **payload bodies are role-gated OPERATOR+**; secret-name denylist renders `«redacted»`; append-only DB role (INSERT/SELECT-only, REVOKE UPDATE/DELETE) + guard trigger + per-row hash chaining, walked and recomputed by an ADMIN-only, audited verifier (`GET /api/admin/audit/integrity`, issue #304). Stated precisely, not overclaimed: the chain is unkeyed SHA-256, so it detects tampering by anyone who cannot recompute the forward hashes — in practice, anyone without direct superuser write access to the audit Postgres — but NOT an actor with full DB write access, who can disable the guard trigger, alter a row, and re-derive every hash after it undetected. HMAC keying would close that gap (issue #311, PO-gated, out of scope here); secrets never persisted. |
...
| **Breach notification** (Art. 24, to the FDPIC) | `audit_insert_failures_total`, the hash-chain integrity verifier (`GET /api/admin/audit/integrity`, walked periodically per RUNBOOK — an unrun chain proves nothing, issue #304), and access logging give the detection substrate — with the SAME qualification as Art. 8 above: it detects tampering by an actor who cannot recompute the forward chain, not one with full DB write access; the notification duty and its clock belong to the controller's incident process. |
