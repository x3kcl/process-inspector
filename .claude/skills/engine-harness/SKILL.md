...
(`inspector:harness`, NOT rest-admin/test). Needs a ghcr login for `ghcr.io/x3kcl/flap`, or
point `PI_FLAP_IMAGE` at a locally built one (`docker compose -f ../flap/docker-compose.base-test.yml build app`).
