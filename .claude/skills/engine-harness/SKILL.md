...
(`inspector:harness`, NOT rest-admin/test). Needs a ghcr login for `ghcr.io/x3kcl/flap` (a PRIVATE package in a private repo — a PAT with
`read:packages`, not the workflow GITHUB_TOKEN, which gets 403), or point `PI_FLAP_IMAGE` at an
image built from a sibling checkout (`docker build -t flap-harness:local ../flap`). In CI this is
the `integration-flap` job, gated on the `GHCR_PULL_TOKEN` secret existing — without it the job
does not run and says so, rather than reporting green without an engine.
...
needs the FULL matrix up (flowable-6 + flowable-7 + legacy; + flap for BootLayoutEngineIT) — a down engine fails loudly
