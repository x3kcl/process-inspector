...
5. Seed run cohorts over engine REST — DEDUPE BY BUSINESS KEY FIRST (issue #227, F-G10):
   ...
   - uxrun-m4-1..8: eight demoFailingPayment instances on engine-a (amount 100, divisor 0) with those businessKeys — they dead-letter in ~40s. The cohort must SPAN TWO DEPLOYED VERSIONS of demoFailingPayment (issue #233 testability: with a single version, "drill link correctly scoped to one version" and "drill link silently shows all versions" are indistinguishable). FIRST check GET /repository/process-definitions?key=demoFailingPayment on engine-a: if fewer than 2 versions exist, POST ${REPO}/docker/processes/demo-failing-payment.bpmn20.xml to /repository/deployments once more (same multipart deploy seed.sh uses; Flowable REST does not dedupe deployments, so re-deploying the identical file bumps the version — the same mechanism demo-migration-v1/v2 rely on). Record the OLDEST and NEWEST processDefinitionId for the key, then start the instances BY processDefinitionId (not by key, which always picks latest): uxrun-m4-1..5 on the NEWEST version, uxrun-m4-6..8 on the OLDEST — expected per-version failure split 5 vs 3. On a reused stack where survivors were REUSED via the businessKey dedupe above: check each survivor's processDefinitionId, and if the eight do not span both versions, terminate uxrun-m4-6..8 and recreate them on the OLDEST version so the 5-vs-3 split is restored (note the cleanup in notes).
