...
describe('curated leak views (R-BAU-02) — honest per R-SEM-05', () => {
  ...
  it('every leak view decodes through the Stage-1 URL codec — a view IS a replayable search', () => {
    for (const view of LEAK_VIEWS) {
      expect(decodeSearch(new URLSearchParams(view.search('vacationRequest', '2026-07-01T00:00:00.000Z')))).not.toBeNull()
    }
  })
  ...
  it.each([
    ['activeOver30d'] as const,
    ['activeOver90d'] as const,
  ])('the %s active leak view is ACTIVE + startedBefore, scoped to one definition', (id) => {
