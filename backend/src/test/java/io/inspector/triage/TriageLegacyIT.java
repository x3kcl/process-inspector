...
class TriageLegacyIT {
    ...
    void allTriageLegsHoldOnThePreCliffEngine() {
        // ?refresh=true is throttled to one cache bypass per 10s (TriageService), and a
        // THROTTLED call silently serves the previous snapshot — which predates the two
        // instances seeded in @BeforeAll whenever a sibling suite consumed the bypass
        // budget moments earlier (issue #333: deterministic 69-vs-71 on the hp02 runners'
        // tighter class schedule). Poll until a genuine bypass lands. This does not
        // soften the pin: a silently-dropped 'suspended' flag double-counts the runtime
        // set on EVERY snapshot, so that regression still fails the bound.
        java.util.concurrent.atomic.AtomicReference<JsonNode> settled =
        ...
        await().atMost(30, TimeUnit.SECONDS).pollInterval(2, TimeUnit.SECONDS).untilAsserted(() -> {
            ...
            // The suspended filter genuinely filters on 6.3.1 (not silently
            // dropped): if the engine ignored the flag, BOTH counts would be the
            // full runtime total and their sum would be twice it. Reconcile
            // against the unfiltered total, fetched directly from the engine
            // right after the (refresh-bypassed) aggregation ran.
