...
public record InspectorProperties(
    ...
    public record EngineConfig(
            @NotBlank
            @Pattern(regexp = ENGINE_ID_PATTERN,
                     message = "engine id must match " + ENGINE_ID_PATTERN)
            String id,
            String name,
    ...
    public enum EngineEnvironment { DEV, TEST, PROD }

    /** Rollout ramp (R-GOV-04): READ_ONLY engines reject every mutating verb in the BFF. */
    public enum EngineMode { READ_WRITE, READ_ONLY }
    ...
    public record Timeouts(Integer connectMs, Integer readMs, Integer writeMs) {
        ...
        public int write()   { return writeMs != null ? writeMs : read(); }
    }
    ...
    public record AlarmThresholds(Integer oldestJobWarnMin, Integer oldestJobCritMin, Integer overdueTimerGraceS) {
        public int oldestJobWarnMinOrDefault()   { return oldestJobWarnMin != null ? oldestJobWarnMin : 5; }
        public int oldestJobCritMinOrDefault()   { return oldestJobCritMin != null ? oldestJobCritMin : 15; }
        public int overdueTimerGraceSOrDefault() { return overdueTimerGraceS != null ? overdueTimerGraceS : 60; }
    }
}
